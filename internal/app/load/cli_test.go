...
func Test_getDataDestination(t *testing.T) {
	...
	Inject(
		...
		&table.MockStorage{},
		&id.MockStorage{},
		map[string]load.DataDestinationFactory{},
		&load.MockRowIterator{},
	)
	...
	tests := []struct {
		...
		struct {
			...
			want:  nil,
			want1: &load.Error{Description: "'connector-ro' is a read only dataconnector"},
		},
	}
